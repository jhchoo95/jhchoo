...
int nand_er(int bank, int pos, int pages)
{
	...
	for(a=pos; a<pos+pages; a++)
	{
		...
		if(r[a][0]!=0)
		{
			...
			if(c==0)
			{
				for(b=pos+pages; b<bank*32*32+32*32; b++)
				{
					if(r[b][0]==0)
					{
						memset(r[b],r[a][0],sizeof(u32)*8);
...
{	
	...
	if(2048-s.host_write-s.gc_write+32*s.gc<400)
	{	
...
void garbage_collection(u32 bank)
{
	...
	for(a=bank*32*32; a<bank*32*32+32*32; a+=PAGES_PER_BLK)
	{
		c=0;
	...
	nand_er(bank, pos, PAGES_PER_BLK);
